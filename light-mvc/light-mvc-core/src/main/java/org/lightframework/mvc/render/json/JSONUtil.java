...
class JSONUtil {
	...
	private static void initMethod(Map<String, AccessDescriptor> propertyMap, Method m, Class<? extends Object> type, boolean write, String name) {
		if (name.length() > 0) {
	...
	private static void initProperty(Map<String, AccessDescriptor> propertyMap, Method m) {
	...
	private static Type getParameterizedType(Type type, Class<?> destClass, int paramIndex) {
		Class<?> clazz = null;
	...
	public static Class<? extends Object> getPropertyType(Type type, Object key) {
		...
		if (clazz != null) {
			if (clazz.isArray()) {
				...
				AccessDescriptor pd = getPropertyDescriptor(clazz, String.valueOf(key));
				if (pd != null) {
					return pd.type;
				}
			}
	...
	public static Object getValue(Object context, Object key) {
		if (context != null) {
			try {
				...
				AccessDescriptor pd = getPropertyDescriptor(context.getClass(), String.valueOf(key));
				if (pd != null) {
	...
	private static AccessDescriptor getPropertyDescriptor(Class<? extends Object> type, String key) {
	...
	public static void setValues(Object base, Map<String, Object> attributeMap) {
		for (String key : attributeMap.keySet()) {
			setValue(base, key, attributeMap.get(key));
		}
	}
	...
	public static void setValue(Object base, Object key, Object value) {
		if (base != null) {
			try {
				...
				AccessDescriptor pd = getPropertyDescriptor(base.getClass(), String.valueOf(key));
				if (pd != null) {
	...
	public static Number toValue(Number value, Class<? extends Object> type) {
		if (type == Long.class) {
			...
			Class<? extends Object> clazz = toWrapper(type);
			if (clazz == type) {
				return null;
			} else {
